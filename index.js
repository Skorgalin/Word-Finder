...
app.post("/admin/switch", (req,res)=>{
  const {oldInput,newInput,email} = req.body;
  if (email!==ADMIN_EMAIL) return res.json({ok:false, msg:"Kein Admin"});
  if (!items[oldInput]) return res.json({ok:false, msg:"Alt fehlt"});
