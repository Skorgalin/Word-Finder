...
if(!fs.existsSync(DATA_FILE)){
  fs.writeFileSync(DATA_FILE, JSON.stringify({ users:{}, items:{} }, null,2));
}
...
app.post("/register",(req,res)=>{
  ...
  res.json({ok:true,admin}); // sofort ins Game einloggen
});
...
app.post("/login",(req,res)=>{
  ...
  if(!data.users[email] || data.users[email].password!==password) return res.json({ok:false});
...
app.post("/submitItem",(req,res)=>{
  ...
  const data=readData();
  let output, firstDiscovery=false;
  ...
  // Admin bekommt keine First Discovery
  if(email===ADMIN_EMAIL) firstDiscovery=false;
...
app.post("/adminSearch",(req,res)=>{
  ...
  for(const [input,obj] of Object.entries(data.items)){
    if(obj.output===query) return res.json({ok:true,input,output:obj.output});
  }

  const newInput = "admin_" + Math.random().toString(36).slice(2,7);
