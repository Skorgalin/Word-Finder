...
const ADMIN_PASSWORD_HASH = crypto.createHash("sha256").update("X9$QvR7!eM2#A8wK@C").digest("hex");
...
function randomOutput() {
  ...
  do {
    ...
    if (type === 0) {
      ...
      out = "";
      for (let i = 0; i < len; i++) out += syllables[Math.floor(Math.random() * syllables.length)];
    } 
    else if (type === 1) {
      // Buchstabenfolge
...
app.post("/register", (req, res) => {
  ...
  if(!isValidEmail(email)) return res.json({ ok: false, error:"Ungültige E-Mail" });
...
app.post("/admin/search", (req, res) => {
  ...
  res.json({ input, output: data.items[input].output });
});
...
app.post("/admin/update", (req, res) => {
  ...
  if (!data.items[input]) return res.json({ ok:false });
